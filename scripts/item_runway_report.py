...
situation as the SOH report's "Stock Summary" -- see soh_daily_report.py),
so this is built from the underlying transactional data instead:
...
this automation (per the project's testing convention):
...
def compute_week_buckets(today, num_periods=NUM_PERIODS):
    """Returns `num_periods` (start, end) date tuples, oldest first, ending
    with the most recent full Monday-Sunday week before `today`'s week."""
    current_week_monday = today - timedelta(days=today.weekday())
    previous_week_end = current_week_monday - timedelta(days=1)

    buckets = []
    for i in range(num_periods):
        week_end = previous_week_end - timedelta(weeks=i)
...
def main():
    ...
    buckets = compute_week_buckets(today)
